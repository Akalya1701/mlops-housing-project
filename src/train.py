...
# Split data: exclude target and sensitive column from features
X = df.drop(columns=["MedHouseVal", "income_group"])
y = df["MedHouseVal"]
sensitive_feature = df["income_group"]

# Train/test split (include sensitive column)
...
# ✅ Manual Fairness Evaluation (group-wise MAE)
